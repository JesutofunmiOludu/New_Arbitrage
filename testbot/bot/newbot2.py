...
import logging
import traceback
import sys # <--- NEW: For logging handler
from termcolor import colored # <--- NEW: For professional colored logging

# --- Configuration & Logging Setup (PRO STYLE) ---
# Replace the old basic logging setup with the advanced formatter
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if "ARBITRAGE OPPORTUNITY DETECTED" in msg:
            # Extract the custom log message after the tag
            ...
            # The timestamp is colored but not part of the box
            return f"{colored(timestamp, 'cyan')} {formatted}"
        
        # Price Update Box (Cyan)
        elif "PRICE UPDATE" in msg:
            # Extract the custom log message after the tag
            message_body = msg.replace("PRICE UPDATE", "").strip()
            formatted = (
                "\n" + "┌" + "─" * 50 + "┐\n" +
                "│" + colored("📊 PRICE UPDATE".center(50), 'cyan', attrs=['bold']) + "│\n" +
                "└" + "─" * 50 + "┘\n" + message_body
            )
            return f"{colored(timestamp, 'cyan')} {formatted}"
...
try:
    # This import relies on your local project structure
...
class ArbitrageMonitor:
    def __init__(self, rpc_url: str, private_key: str, contract_address: str):
        ...
        self.dexes = {
            ...
            'aerodrome': DEXInfo('Aerodrome', '0xcF77a3Ba9A5CA399B7c97c74d54e5b1Beb874E43', '0x420DD381b31aEf6683db6B902084cB0FFECe40Da')
        }
    ...
    # present_scraped_tokens (TOKEN LOADING UNCHANGED, AS REQUESTED)
    def present_scraped_tokens(self, limit=50):
        ...
        # --- BASESCAN SCRAPING LOGIC REMAINS HERE ---
    ...
    # get_token_price (V2 PRICING LOGIC REMAINS HERE)
    async def get_token_price(self, token_address: str, stable_token: str, dex_key: str) -> Optional[float]:
        """Determines price using V2 getAmountsOut function."""
        try:
            ...
            # V2 Logic: amounts_out[1] is the amount of tokenB received for tokenA
    ...
    async def execute_arbitrage(self, opportunity: ArbitrageOpportunity):
        ...
        try:
            # Use standard logger.info here, as the custom formatter is applied globally
    ...
    # check_and_execute_opportunity (ENHANCED REPORTING)
    async def check_and_execute_opportunity(self, token: dict, stable_token: str = 'USDC'):
        token_address = token['address']
        prices = {}
        # Gather prices from the cache updated by the polling loop
        ...
        # Increased lower threshold to avoid gas fee losses (UNCHANGED)
        ...
        # Log potential opportunity, similar to the original log
        logger.info(
        ...
        if net_profit_usd > self.min_profit_threshold:
            ...
            # --- PROFESSIONAL ARBITRAGE REPORTING ---
            log_msg = (
                f"Pair: {token['symbol']}/USDC (V2 ARB)\n"
            ...
            try:
                ...
                while self.is_monitoring:
                    ...
                    for event in entries:
                        # Event detection is used as a trigger to re-check the price
    ...
    # price_polling_loop (ENHANCED REPORTING)
    async def price_polling_loop(self):
        ...
        while self.is_monitoring:
            for token in self.selected_tokens:
                prices = {}
                log_details = "" # For visual log output
                ...
                for dex_key in self.selected_dexes:
                    ...
                    if price:
                        ...
                        log_details += f"\n  • {self.dexes[dex_key].name:<20}: ${price:.6f}" # Add to log
                
                # --- VISUAL PRICE REPORTING ---
                if len(prices) >= 2:
                    min_price_dex, min_price = min(prices.items(), key=lambda item: item[1])
                    max_price_dex, max_price = max(prices.items(), key=lambda item: item[1])
                    price_diff_percent = ((max_price - min_price) / min_price) * 100
                    
                    # Only print the price update box if there's a minor spread
                    if price_diff_percent > 0.1: 
                        msg = (f"Token: {token['symbol']}\n"
                               f"Spread: {price_diff_percent:.4f}%\n"
                               f"{log_details}")
                        logger.info("PRICE UPDATE" + msg) # Custom tag for coloring
    ...
    # monitor_opportunities (UNCHANGED)
...
async def main():
    print("🚀 Arbitrage Monitor Bot v3.1 PRO (V2 Logic, Live Trading)")
    print("=" * 60)
    print(colored("⚠️  WARNING: This bot is configured for LIVE trading. ⚠️", 'red', attrs=['bold']))
    print(colored("⚠️  Ensure your contract is funded and you understand the risks. ⚠️", 'red', attrs=['bold']))
    print("=" * 60)
    ...
    try:
        ...
        contract_address = os.getenv('CONTRACT_ADDRESS', input("Enter your deployed contract address: ").strip())
